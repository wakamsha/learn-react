...
export const Navigation = () => {
  const linkData = Object.fromEntries(Object.entries(routes).filter(([, value]) => !value.includes(':')));
    ...
    <div className={styleBase}>
      ...
      <ul className={styleNavigation}>
        {Object.entries(linkData).map(([key, value]) => (
          <li key={key}>
            <NavLink className={({ isActive }) => cx(styleLink, isActive && styleLinkActive)} to={value}>
              {key}
            </NavLink>
          </li>
        ))}
